...
This project is currently a portfolio backend. It has token authentication, request validation, ownership checks, and restricted CORS configuration, but it still needs additional hardening before handling sensitive production traffic.
...
1. Move blog and comment authorization into Laravel policies.
2. Add a data backfill migration for existing blog rows.
3. Add a deployment migration plan for databases that already ran the old blog schema.
4. Add automated tests for unauthorized access, invalid payloads, rate limiting, and owner-only operations.
